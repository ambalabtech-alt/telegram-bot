...
ORDER_WRITE_LOCK = threading.Lock()
...
async def notify_admin_new_order(msg: Message, st: OrderState):
    text = (
        ...
        f"Номер: <b>{nz(st.order_id)}</b>\n"
        f"Лікар: {(msg.from_user.full_name if msg.from_user else '')}"
        f" {('@' + msg.from_user.username if msg.from_user and msg.from_user.username else '')}\n"
        f"Пацієнт: {st.patient_lastname}\n"
...
async def np_city_pick_cb(q: CallbackQuery):
    ...
    try:
        ...
        return
    st.np_city_ref = ref
    if not await _safe_set_cell(st.sheet_row, 'np_city_ref', ref, q.message): return await q.answer()
    city_name = ''
    for c in getattr(st, 'last_np_cities', []) or []:
        if c.get('Ref') == ref:
            ...
            break
    if city_name:
        if not await _safe_set_cell(st.sheet_row, 'np_city_name', city_name, q.message): return await q.answer()
    st.delivery_step = ''
    st.step = 'await_np_number'
...
async def np_wh_pick_cb(q: CallbackQuery):
    ...
    for w in getattr(st, 'last_np_items', []) or []:
        if w.get('Ref') == ref:
            picked = w
            break
    if not await _safe_set_cell(st.sheet_row, 'np_warehouse_ref', ref, q.message): return await q.answer()
    np_save_current_delivery(q.message.chat.id, st)
    if picked:
        if not await _safe_set_cell(st.sheet_row, 'np_warehouse_desc', picked.get('Description', ''), q.message): return await q.answer()
        if picked.get('CityDescription'):
            if not await _safe_set_cell(st.sheet_row, 'np_city_name', picked.get('CityDescription'), q.message): return await q.answer()
    st.delivery_step = ''
    st.step = 'choose_files_method'
