...
async def _refresh_reply_keyboard(msg: Message, text: str, kb: ReplyKeyboardMarkup, parse_mode: Optional[str] = 'HTML') -> None:
    """Force Telegram clients to reopen a specific reply keyboard."""
    try:
        await msg.answer('‎', reply_markup=ReplyKeyboardRemove())
    except Exception:
        pass
...
async def flow(msg: Message):
    ...
    if st.step == 'await_links':
        ...
        await msg.answer('\u2060', reply_markup=files_aux_kb())
        return
    if st.step == 'await_tele_files':
        if (msg.text or '').strip() == '✅ Готово':
    ...
    if st.step == 'await_notes':
        ...
        if msg.text:
            ...
            await msg.answer('\u2060', reply_markup=done_kb())
            return
