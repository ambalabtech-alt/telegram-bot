...
def _restore_order_state_from_sheet(chat_id: int) -> Optional['OrderState']:
    try:
        head = headers_map(ws)
        idx_chat = head.get('chat_id')
        idx_order = head.get('order_id')
        idx_status = head.get('status')
        if not (idx_chat and idx_order and idx_status):
            return None
        rows = _retry_sheets(ws.get_all_values)
        if len(rows) <= 1:
            return None
        active_statuses_exclude = {'order_submitted', 'cancelled', 'cancelled_by_user', 'auto_cancelled'}
        chosen = None
        for rownum in range(len(rows), 1, -1):
            row = rows[rownum - 1]
            chat_val = row[idx_chat - 1].strip() if idx_chat - 1 < len(row) else ''
            status_val = row[idx_status - 1].strip() if idx_status - 1 < len(row) else ''
            order_val = row[idx_order - 1].strip() if idx_order - 1 < len(row) else ''
            if chat_val == str(chat_id) and order_val and status_val not in active_statuses_exclude:
                chosen = (rownum, row)
                break
        if not chosen:
            return None
        ...
        rownum, row = chosen
        def rv(col_name: str) -> str:
            c = head.get(col_name)
            return row[c - 1].strip() if c and c - 1 < len(row) else ''

        st = OrderState()
        st.sheet_row = rownum
        st.order_id = rv('order_id')
        st.patient_lastname = rv('patient_lastname')
        st.work_type = rv('work_type')
        due = rv('due_date')
        if due:
            try:
                parsed = parse_date_uk(due)
                st.due_date_iso = parsed.isoformat() if parsed else due
            except Exception:
                st.due_date_iso = due
        st.np_city_ref = rv('np_city_ref')
        st.np_warehouse_ref = rv('np_warehouse_ref')
        files_method = rv('files_method')
        files_ids = rv('files_telegram_id')
        links = rv('links_external')
        notes = rv('notes')
        voice = rv('voice_id')

        st.telegram_file_ids = [p.strip() for p in files_ids.split() if p.strip()]
        st.accepted_files_count = len(st.telegram_file_ids)
        st.links_external = [u for u in links.split() if u]
        st.accepted_links_count = len(st.links_external)
        st.accepted_notes_count = int(bool(notes.strip())) + len([v for v in voice.split() if v])

        if not rv('phone'):
            st.step = 'doctor_phone'
        elif not st.patient_lastname:
            st.step = 'patient_lastname'
        elif not st.work_type:
            st.step = 'work_type'
        elif not due:
            st.step = 'due_date'
        elif not files_method:
            st.step = 'np_menu'
        elif 'telegram_upload' in files_method and not st.accepted_files_count:
            st.step = 'await_tele_files'
        elif 'link' in files_method and not st.accepted_links_count:
            st.step = 'await_links'
...
def close_files_wave(st: 'OrderState') -> None:
    ...
    st.last_file_update_ts = st.files_done_cutoff_ts
...
def can_accept_tail_file(st: Optional['OrderState']) -> bool:
    if not st:
        return False
    refresh_file_tail_state(st)
    if not getattr(st, 'file_tail_open', False):
        return False
    base_ts = float(getattr(st, 'last_file_update_ts', 0.0) or getattr(st, 'files_done_cutoff_ts', 0.0) or 0.0)
    if not base_ts:
        return False
    timeout_sec = float(getattr(st, 'file_tail_timeout_sec', FILE_TAIL_TIMEOUT_SEC) or FILE_TAIL_TIMEOUT_SEC)
    return (time.time() - base_ts) <= timeout_sec
...
async def handle_telegram_upload(msg: Message, st: 'OrderState', silent: bool = False, is_tail: bool = False) -> bool:
    ...
    if not FILES_CHANNEL_ID:
        return False
    tail_allowed = bool(is_tail and can_accept_tail_file(st))
    # Do not accept new files after the user pressed Done, unless this is a
    # late Telegram upload that was already in flight when the user tapped Done.
    if st.files_wave_closed and not tail_allowed:
        return False
    ...
    # Update wave state and ensure watcher only for the live upload step.
    # Tail files that finish uploading after Done are accepted silently and
    # must not reopen the wave or send extra acknowledgements.
    if tail_allowed:
...
async def flow(msg: Message):
    ...
    if msg.content_type != 'text':
        ...
        tail_file = st and msg.content_type in (ContentType.DOCUMENT, ContentType.PHOTO) and can_accept_tail_file(st)
        expecting_voice = st and st.step == 'await_notes' and msg.content_type == ContentType.VOICE
        if tail_file:
            handled = await handle_telegram_upload(msg, st, silent=True, is_tail=True)
            if handled:
                return
    ...
    if st.step == 'await_tele_files':
        if (msg.text or '').strip() == '✅ Готово':
            # Telegram Desktop/Web can still be uploading files client-side when
            # the user already presses Done.  In that race we must move forward
            # immediately, keep a short silent tail window open for late files,
            # and never throw those files into notes/offtopic handling.
            close_files_wave(st)
            set_cell(st.sheet_row, 'status', 'files_expected')
            await save_bot_state_async(msg.chat.id, st)
            await ask_notes(msg, st)
